...
void gen_test(int test_id, int subtask) {
    string test_name = "tests/" + string(test_id < 10 ? "0" : "") + to_string(test_id);
...
int main(int argc, char* argv[]) {
    ...
    // Tự động tạo thư mục 'tests'
    std::filesystem::create_directory("tests");
    cout << "Starting generation of 50 test files...\n";

    // Subtask 1: 20% số test (Test 1 -> 10)
    for (int i = 1; i <= 10; ++i) gen_test(i, 1);
    
    // Subtask 2: 40% số test (Test 11 -> 30)
    for (int i = 11; i <= 30; ++i) gen_test(i, 2);
    
    // Subtask 3: 40% số test (Test 31 -> 50)
    for (int i = 31; i <= 50; ++i) gen_test(i, 3);

    cout << "Done! All files are generated inside the 'tests' directory.\n";
    return 0;
}